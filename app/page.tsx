...
export default function Home() {
  ...
  const filteredBlogs = blogs.filter(blogs => {
    const matchCategory = filterCategory === "all" || blogs.status === filterCategory
    const matchSearch = blogs.title.toLowerCase().includes(searchQuery.toLowerCase());
    return matchCategory && matchSearch
  })
  return (
    <MainLayout>
      <h1 className="heading">Blog Management Dashboard</h1>
      <Search onSearch={setSearchQuery} searchQuery={searchQuery} filterCategory={filterCategory} setFilterCategory={setFilterCategory} />
